...
import { BarChart3, Bell, CheckCircle, Loader2 } from "lucide-react"
...
export default function FinishStep() {
  ...
  const handleFinishSetup = async () => {
    ...
    try {
      // TODO API call to save the form data
        ...
        <div className="space-y-8">
            ...
            <div className="space-y-3">
              {notificationOptions.map((notification) => (
                <div key={notification} className="flex items-center space-x-3 p-3 rounded-lg border hover:bg-slate-50">
                  ...
                  />
                  <Label htmlFor={notification} className="text-sm font-normal cursor-pointer flex-1">
                    {notification}
                  </Label>
                </div>
              ))}
          ...
          <div className="bg-blue-50 rounded-lg p-6">
            ...
            <div className="text-sm text-blue-800 space-y-1">
              <p>
                <strong>Resident:</strong> {formData.preferredName} ({formData.relationship})
              </p>
              <p>
                <strong>Companion:</strong> {formData.companionName}
              </p>
              <p>
                <strong>Voice:</strong> {formData.voiceSelection} • <strong>Tone:</strong> {formData.conversationTone}
              </p>
              <p>
                <strong>Reports:</strong> {formData.reportFrequency}
              </p>
            </div>
          </div>
        </div>
